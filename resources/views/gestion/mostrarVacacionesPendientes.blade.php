...
<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Registro de Vacaciones Pendientes') }}
        </h2>
    </x-slot>
    ...
    <div class="py-10">
        <div class="mb-3 py-3 ml-16 leading-normal text-green-500 rounded-lg" role="alert">
            ...
            <div class="text-right">
                <a href="{{ route('mostrarVacaciones.show') }}" class='w-auto mb-10 mr-20 bg-blue-500 hover:bg-blue-600 rounded-lg shadow-xl font-medium text-white px-4 py-2'>
                    Histórico de Solicitudes de Vacaciones
                </a>
            </div>
        ...
        <div class="mx-auto sm:px-6 lg:px-8" style="width:80rem;">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-6" style="width:100%;">
                <p class="text-center content-center items-center text-xl mt-3 font-bold my-3">Solicitudes Pendientes de Vacaciones</p>
                                ...
                                <td class=" px-2 py-1">
                                    <div class="flex justify-center rounded-lg text-lg" role="group">
                                        ...
                                        <form action= "button" id="rechazarForm" class="formEliminar rounded bg-red-600 hover:bg-red-700 ml-2 text-center">
                                            @csrf
                                            <button type="button" id="rechazarButton" class="rounded text-white font-bold py-2 px-2 text-center" data-bs-toggle="modal" data-bs-target="#ModalRechazar_{{$vacacion->id}}">Rechazar</button>
                                        </form>     
                                        {{-- Boton autorizar vacacion --}}
                                        <form action= "button" id="autorizoForm" class="formAutorizar rounded bg-green-600 hover:bg-green-700 ml-2 text-center">
                                            @csrf
                                            <button type="button" id="autorizarButton" class="rounded text-white font-bold py-2 px-2 text-center" data-bs-toggle="modal" data-bs-target="#MyModalAutorizar_{{$vacacion->id}}">Autorizar</button>
                                        </form>              
                                        <!-- Modal -->
                                        <div class="modal fade" id="ModalRechazar_{{$vacacion->id}}" tabindex="-1" aria-labelledby="RechazarModalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content" style="width: 500px; height: 450px;">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="RechazarModalLabel">Confirmar rechazo de la solicitud de vacaciones</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <div style="display: flex; justify-content: center; align-items: center; ">
                                                            <form action="{{ route('rechazarVacacion.reject',['id'=>$vacacion->id]) }}" method="POST" id="rechazoForm">
                                                                @csrf 
                                        ...
                                        <!-- Modal -->
                                        <div class="modal fade" id="MyModalAutorizar_{{$vacacion->id}}" tabindex="-1" aria-labelledby="AutorizarModalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content" style="width: 500px; height: 450px;">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="AutorizarModalLabel">Confirmar autorización de la solicitud de vacaciones</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <div style="display: flex; justify-content: center; align-items: center; ">
                                                            <form action="{{ route('aceptarVacacion.accept',['id'=>$vacacion->id]) }}" method="POST" id="autorizarForm">
                                                                @csrf 
