<div>
    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-5"
        ...
        <div>
            <select wire:model='statusSelect' wire:change='recargarTabla' class="py-2 px-3 rounded-lg mt-4 focus:outline-none focus:ring-2 focus:border-transparent" >
                <option value=0>Todos los estados</option>
                @foreach ($estados as $stds)
                    <option value={{$stds->id}}>{{$stds->nombre}}</option>
                @endforeach
            </select>
            <select wire:model='zonaSelect' wire:change='recargarTabla' class="py-2 px-3 rounded-lg mt-4 focus:outline-none focus:ring-2 focus:border-transparent" >
                <option value=0>Todas las zonas</option>
                @foreach ($zonas as $zns)
                    <option value={{$zns->area_clave}}>{{$zns->area_nombre}}</option>
                @endforeach
            </select>
        </div>
                    ...
                    <tr data-id="{{ $resguardo->n_factura }}">
                        ...
                        <td class="border-l px-1 py-2">
                            <div class="flex justify-center rounded-lg text-small" role="group">
                                ...
                                <a href="{{ route('resguardos.generarhijo', $resguardo->id) }}" style="text-decoration:none;"
                                class="rounded bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-2.5 mx-2 ml-2"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ffffff" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H3a2 2 0 0 0-2 0v16c0 1.1.9 2 2 2h12a2 2 0 0 0 2-2V8l-6-6z" /><path d="M14 2H9a2 2 0 0 0-2 0v16c0 1.1.9 2 2 2h12a2 2 0 0 0 2-2V8l-6-6z"/></svg></a>
                                @endcan
