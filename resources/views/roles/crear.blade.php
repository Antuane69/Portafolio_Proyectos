<x-app-layout>
    ...
    @section('css')
        <link rel="stylesheet" t ype="text/css" href="{{ asset('css/bootstrap.min.css') }}">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <center>
                    <form action="{{ route('roles.store') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <br>
                        <table id="data-table" class="stripe hover translate-table center" style="width:50%;">
                            <thead>
                                <tr>
                            ...
                            @foreach ($permissions as $permission)
                                <tr data-id="{{ $permission->name }}">
                                    <td><input type="checkbox" name="permisos[]" id="permisos" class="check"
                                            value="{{ $permission->id }}">
                                        {{ $permission->name }} </td>
                ...
                form.addEventListener('submit', function(event) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            this.submit();
                            Swal.fire('¡Enviado!',
                                'El registro del rol se ha sido enviado exitosamente.',
                                'success');
