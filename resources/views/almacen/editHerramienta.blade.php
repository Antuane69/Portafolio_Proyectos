<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __("Registro de Herramienta (Modo de Edición)") }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        ...
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl md:rounded-lg">
                <form id="formulario" action={{ route('editarHerramientas.store',$id) }} method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class='flex items-center justify-center  md:gap-8 gap-4 pt-3 pb-2 font-bold text-3xl text-slate-700 rounded-t-xl mx-10 mt-5' style="background-color: #FFFF7B">
                        <p>
                            Registro de Herramientas (Modo de Edición)
                        </p>
                    </div>                    
                    ...
                    <div class="mx-10 mb-5 pb-10 text-center rounded-b-xl rounded-l-xl rounded-r-xl bg-gray-100">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-8 mx-7 mt-5"> 
                            <div class='grid grid-cols-1'>
                                <div class='grid grid-cols-1' onclick="openInput()" id="imageContainer1">
                                    ...
                                    @if ($herramienta->imagen != '')                                                               
                                        <div style="width: 220px; height: 180px;" name='img' class='bg-white mt-2 text-center border-yellow-200 overflow-hidden mx-auto focus:outline-none focus:ring-2  focus:border-transparent p-2 w-5/6 rounded-lg border-2'>
                                            <img id="imgPreview1" style="width: 200px; height: 160px;" src="{{ asset('/img/almacen/Herramientas/' . $herramienta->imagen) }}">
                                        </div>
                                    @else
                                        <div style="width: 320px; height: 200px;" name='img' class='bg-white mt-2 text-center border-yellow-200 overflow-hidden mx-auto focus:outline-none focus:ring-2  focus:border-transparent p-2 w-5/6 rounded-lg border-2'>
                                ...
                                <p class="bloack uppercase text-gray-800 font-bold mb-6">
                                    * Código y Descripción
                                    <textarea id="descripcion" name="descripcion"
                                        ...
                                        required placeholder="Ingrese el código y la descripción de la herramienta">{{ old('descripcion',$herramienta->descripcion) }}</textarea>
                            ...
                            <div class='grid grid-cols-1'>
                                ...
                                <p>
                                    <select id="area" name="area" id="usados_talla" class='w-5/6 mb-1 p-2 px-3 rounded-lg border-2 mt-1 focus:outline-none focus:ring-2 focus:border-transparent' required>             
                                        <option value="" disabled selected>Seleccione una Opcion</option>
                                        @foreach($opciones as $opcion)
                                            @if ($herramienta->area == $opcion)
                            ...
                            <div class='grid grid-cols-1'>
                                ...
                                <p>
                                    ...
                                    required oninput="validarCantidad()" value="{{$herramienta->precio}}">
                            ...
                            <div class='grid grid-cols-1'>
                                ...
                                <p>
                                    ...
                                    required oninput="validarCantidad()" value="{{$herramienta->cantidad}}">
...
<script>
    $(document).ready(function() {
        ...
        // Obtener el array de nombres seleccionados
        var nombresSeleccionados = @json($nombresSeleccionados);

        // Recorrer el array de nombres seleccionados
        nombresSeleccionados.forEach(function(nombre) {
            // Seleccionar la opción correspondiente en el campo de selección múltiple
            $('.js-example-basic-multiple').append($('<option selected></option>').val(nombre).html(nombre));
        });

        // Actualizar el campo de selección múltiple con los nombres seleccionados
